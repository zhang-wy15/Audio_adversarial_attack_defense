...
def compute_PSD_matrix(audio, window_size):
    ...
    #z = np.sqrt(8.0/3.) * abs(win / window_size)
    
    window = torch.hann_window(2048)
    stft = torch.stft(torch.FloatTensor(audio), n_fft=2048, hop_length=512, win_length=2048, window=window, center=False)
    spect = torch.sqrt(torch.pow(stft[:,:,0], 2) + torch.pow(stft[:,:,1], 2))
    z = np.sqrt(8. / 3.) * torch.abs(spect / window_size)
...
def compute_th(PSD, barks, ATH, freqs):
    ...
    if 0 in masker_index:
        masker_index = np.delete(masker_index, 0)
    if length - 1 in masker_index:
        masker_index = np.delete(masker_index, length - 1)
    ...
    #L_TM = model1(bark_psd, delta_TM, barks) 
    L_TM = np.array(L_TM)
